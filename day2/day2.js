...
// first part is number of times the letter must appear 1-8
// the second part is the letter to test
// the third part is the password
let workingPasswords = []

for (let passwordTest of passwordTests) {
  let [test, letter, password] = passwordTest.split(" ")
  let [minAppearances, maxAppearances] = test.split("-").map(Number)
  letter = letter.replace(":", "")

  let regex = new RegExp(letter, "g")
  let matches = password.match(regex) || []
  let numberOfApperances = matches.length

  if (numberOfApperances >= minAppearances && numberOfApperances <= maxAppearances) {
